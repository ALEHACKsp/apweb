...
if ($is_hwid)
{
	...
	if ( $user_hwid && ValidUserHWID($user_hwid) && UserLicenseValid($user_hwid) )
	{
		...
		if ( $all_build_count > 0 )
		{
			...
			{
				$page_info = "[".$hack_name."] Вы уже обновляли чит.";
			}
			else
			{
				$page_info = "[".$hack_name."] Нажмите кнопку для скачивания";
		...
		{
			$page_info = "[".$hack_name."] Версий больше не осталось.";
		}
	}
	else
	{
...
<body>
	...
	<li>
		...
		<p class="days_ref">Всего уникальных версий</p>
		<a href="<?php echo $user_buidl_url; ?>" class="btn btn-primary btn-sm">скачать</a>
	</li>
</ul>
